  ...
  firebase.initializeApp(firebaseConfig);
      ...
      btnpicles.addEventListener("click",()=>{
        ...
        pagante.get().then((doc) => {
          var quantidade  = doc.data().reais;
          if(quantidade < valuepicles){
            alert("Você não tem dinheiro")
          }else{
            recibo.get().then((doc) => {
              if (doc.exists) {
                // Obtenha o valor de um campo específico
                var namerecibo = doc.data().userName;
                  ...
                  pagante.get().then((doc) => {
                      if (doc.exists) {
                        ...
                        console.log(valordois);
                          ...
                          alert("Valor transferido para " + namerecibo);
                          document.getElementById("uidpicles").value = "";
                          document.getElementById("valuepicles").value = "";
  
                      } else {
                        alert('O documento não existe');
                      }
                    }).catch((error) => {
